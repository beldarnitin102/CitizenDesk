...
function EmployeePerformanceTable({ employees = [] }) {
  return (
    <Card className="rounded-3xl p-6">
      <h2 className="mb-6 text-xl font-bold">Employee Performance</h2>
            ...
            <tbody>
              {employees.map((emp) => {
                const rate =
                  emp.totalAssigned === 0
                    ? 0
                    : ((emp.resolved / emp.totalAssigned) * 100).toFixed(1);
                ...
                return (
                  <tr key={emp._id} className="border-b hover:bg-slate-50">
                    ...
                    <td className="px-4 py-4 text-center">{emp.resolved}</td>

                    <td className="px-4 py-4 text-center">{emp.inProgress}</td>
